...
class CommonComments extends Component {
  ...
  handleSubmit(ev){
    ...
    fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=comment&userid=" + localStorage.userid + "&uniquekey=" + this.props.uniquekey + "&commnet=" + formdata.remark, myFetchOptions)
      .then(response => response.json())
      .then(json => {
  			this.componentDidMount();                    //重新加载页面的数据
		  });
  }
    ...
    var myFetchOptions = {
		...
		fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=uc&userid="+localStorage.userid+"&uniquekey="+this.props.uniquekey, myFetchOptions)
			...
			.then(json=>{
				//收藏成功以后进行一下全局的提醒
				//console.log(json);
				if(json) notification['success']({message: 'ReactNews提醒', description: '收藏此文章成功'});
			});
  }
        ...
        <Row>
          <Col span={24}>
            ...
            <Form onSubmit={this.handleSubmit.bind(this)}>
              <FormItem label="您的评论">
                <Input type="textarea" placeholder="随便写您的评论..." {...getFieldProps('remark',{initialValue:''})}/>
              </FormItem>
