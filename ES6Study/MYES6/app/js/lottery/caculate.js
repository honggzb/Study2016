class Calculate {
   ...
   * computeCount - 计算注数  
   *    
   * @param  {number} active    当前选中的号码01， 02， 。。。，11
   * @param  {string} play_name 当前的玩法标识 R2(任二)，R3(任三)。。。，R8(任八)
  ...
  static combine(arr,size){
    let allResult = [];
    (function f(arr, size, result) {    //iife function, 递归， 注意ES6写递归时候，匿名函数必须有函数名
      ...
      if(size === arrLen){
        ...
        for(let i=0;i<arrLen; i++){
          let newResult = [].concat(result);   //空数组用于初始化
