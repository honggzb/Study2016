...
class Base {
  ...
  initNumber(){
    for(let i=1;i<12;i++){
      this.number.add((''+i).padStart(2,'0'));    //个位数前面要补零： padStart(2,'0') ->每个保持2位，如不满足2位，前面补0
    }
  }
  ...
  setOmit(omit){
    let self = this;
    self.omit.clear();   //先清空当前遗漏数据， Map.clear()
    for(let [index,item] of omit.entries()){
      self.omit.set(index, item);
    }
    $(self.omit_el).each(function(index,item){    //将遗漏数据布置到页面
  ...
  setOpenCode(code){
    let self = this;
    self.open_code.clear();   //先清空当前开奖号码， Map.clear()
    for(let index of open_code.values()){
